...
Single-workspace Terraform configuration that manages one Cloudflare account, one zone, and its DNS records via the `cloudflare/cloudflare` provider (~> 5.18.0). This is not a reusable module — `main.tf` declares concrete resources directly.
